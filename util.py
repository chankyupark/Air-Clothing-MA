...
def write1(x, img, phrases, coco_classes, colors):
    ''' write
    '''
    c1 = tuple(x[1:3].int())
    c2 = tuple(x[3:5].int())
    ...
    if cls == 0:  # for human bbox only detection
        ...
        for i, phrase in enumerate(phrases):
            cv2.putText(
                img,
                phrase,
                (c1[0] - 200, c1[1] + t_size[1] + 150 + i * 20),
                cv2.FONT_HERSHEY_PLAIN,
                1,
                color,
                1,
...
def write(x, img, phrases, order, coco_classes, colors):
    ''' write phrases on image
    '''
    c1 = tuple(x[1:3].int())
    c2 = tuple(x[3:5].int())
    cls = int(x[-1])
...
def write_results(prediction, confidence, num_classes, device, nms=True,
    ...
    for ind in range(batch_size):
        ...
        # Add the class index and the class score of class having maximum
        #  score
            ...
            if cls == 0:
                ...
                if nms:
                    ...
                    for i in range(idx):
                        ...
                        # Zero out all the detections that have IoU >
                        # treshhold
                        ...
                        image_pred_class = \
                            image_pred_class[non_zero_ind].view(-1, 7)

                # Concatenate the batch_id of the image to the detection
                # this helps us identify which image does the detection
                # correspond to
                # We use a linear straucture to hold ALL the detections
                # from the batch
