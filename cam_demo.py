...
def counter_parts1(sentence):
    ''' return each counting bin of classification results :: upper
    '''
    ...
    return color_bin, pattern_bin, gender_bin, season_bin, class_bin, \
        sleeves_bin


def counter_parts2(sentence):
    ''' return each counting bin of classification results :: lower
    '''
    color_bin = np.zeros(14)
    pattern_bin = np.zeros(6)
    gender_bin = np.zeros(2)
...
def keep_majority(
    ...
    ''' return majoriy classes within some period
    '''
    ...
    (
        ...
        class_bin,
        sleeves_bin,
    ) = counter_parts1(tokens[0])  # for tokens[0] ==> upper
    if tokens.__len__() == 2:
        (
            color_bin,
            pattern_bin,
            ...
            class_bin,
            sleeves_bin,
        ) = counter_parts1(tokens[1])  # for tokens[1] ==> lower
